...
@if (! $href)
    <button
        {{ $attributes->merge(['class' => $variantStyles[$variant] . ' inline-flex items-center gap-2 justify-center rounded-md py-2 px-3 text-sm outline-offset-2 transition active:transition-none']) }}
    >
        {{ $slot }}
    </button>
@else
